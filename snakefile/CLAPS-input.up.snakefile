# up-analysis For INPUT samples

include: "conf/config"
SAMPLES=[line.rstrip() for line in open("conf/sm.Input.list")]
...
rule bedSort:
	input:
		"{sample}_damage.rmdup.bed"
	output:
		temp("{sample}_damage.rmdup_sorted.N.bed")
	shell:
...
rule bgzip:
	input:
		rules.bedSort.output
	output:
		"{sample}_damage.rmdup_sorted.N.bed.gz"
	shell:
...
rule tabix:
	input:
		rules.bgzip.output
	output:
		"{sample}_damage.rmdup_sorted.N.bed.gz.tbi"
	shell:
		"{tabix} {input}"
...
rule bedtobam:
	...
	output:
		bam="{sample}_damage.rmdup_sorted.N.bam",
		bai="{sample}_damage.rmdup_sorted.N.bam.bai"
	shell:
		"bedtools bedtobam"
		" -g {Genome}"
		" -mapq 60"
...
rule bamtobw:
	...
	output:
		"{sample}_damage.rmdup_sorted.N.rpkm.bw"
	threads: 6
	shell:
